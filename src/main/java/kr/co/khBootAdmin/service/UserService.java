...
import kr.co.khBootAdmin.exception.UserLogoutException;
import kr.co.khBootAdmin.mapper.AdminMapper;
import kr.co.khBootAdmin.mapper.UserAuthorityMapper;
import kr.co.khBootAdmin.model.CustomUserDetails;
import kr.co.khBootAdmin.model.Role;
import kr.co.khBootAdmin.model.User;
import kr.co.khBootAdmin.model.UserDevice;
import kr.co.khBootAdmin.model.payload.request.LogOutRequest;
import kr.co.khBootAdmin.model.payload.request.RegistrationRequest;
import kr.co.khBootAdmin.model.payload.request.UserRegisterRequest;
import kr.co.khBootAdmin.model.payload.response.PagedResponse;
...
import kr.co.khBootAdmin.repository.UserRepository;
import kr.co.khBootAdmin.util.ModelMapper;
import kr.co.khBootAdmin.util.ValidatePageNumberAndSize;
...
public class UserService {
    ...
    @Autowired
    private UserAuthorityMapper userAuthorityMapper; // ✅ Mapper 주입
    ...
    public List<User> getAllUserList() {
        ...
        for(User item:list) {
            List<Role> roleList = userAuthorityMapper.findRolesByUserId(item.getId());
            Set<Role> roleSet = new HashSet<>(roleList);
            item.setRoles(roleSet);
        }
        return list;
    }
...
//            }
    ...
    public void deleteUserById(Long userId) {
        ...
        // 실제 삭제 처리 (MyBatis 호출)
        adminMapper.deleteUserAuthorityByUserId(userId);
        adminMapper.deleteUser(userId);
    }

    public List<TrackVO> selectCount(TrackVO trackVO) {
        return adminMapper.selectCount( trackVO );
    }

    public List<UserRoleVO> searchUsers (String keyword) {
    return adminMapper.searchUsers( keyword );
    }

    public void activeUserById(Long userId) {
        adminMapper.activeUser(userId);
    }

    public void deActiveUserById(Long userId) {
        adminMapper.deActiveUser(userId);
    }
